...
type Spec struct {
  ...
  AdminClient *AdminClient `json:"adminClient,omitempty"`
...
func (strct *Spec) MarshalJSON() ([]byte, error) {
	...
	buf.WriteString("{")
    comma := false
    // Marshal the "adminClient" field
    if comma { 
        buf.WriteString(",") 
    }
    buf.WriteString("\"adminClient\": ")
	if tmp, err := json.Marshal(strct.AdminClient); err != nil {
		return nil, err
 	} else {
 		buf.Write(tmp)
	}
...
func (strct *Spec) UnmarshalJSON(b []byte) error {
    ...
    for k, v := range jsonMap {
        switch k {
        case "adminClient":
            if err := json.Unmarshal([]byte(v), &strct.AdminClient); err != nil {
                return err
             }
        case "authentication":
